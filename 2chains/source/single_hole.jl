...
# Fields
*   `size::Int64`: number of lattice sites in each chain
...
function run(input::Union{Missing, OrderedDict} = missing; howmany = 3, factor = true)
    system::System = if input === missing
        ...
        System(
            ...
            input["primary coupling type"],
            input["secondary coupling type"],
            input["primary magnon interaction"],
            input["secondary magnon interaction"],
            input["primary proximity interaction"],
            input["interchain coupling"],
            input["interchain coupling type"],
            input["interchain magnon interaction"],
            input["interchain proximity interaction"],
            input["hopping constant"],
            input["periodic along Y"],
            input["solver type"]
        )
    end
    println()
...
function makeBasis(system::System)::Basis
    ...
    #     # comment: system.size is length of a chain, so in the lattice there are 2 chains each with length system.size
    # end
    if iseven(system.magnetization) || system.magnetization < 0 || system.magnetization > 2*system.size - 1
        error("Wrong magnetization sector in the input file! Possible subspaces are odd integers from -2*system.size + 1 to 2*system.size - 1")
        # comment:  system.magnetization = abs(number of spins up - number of spins down)
        #           must be odd for single hole case in lattice with even number of sites
        #           we always have even number of sites in 2 chains of the same size limit
    end

    ### Note: in the calculated basis representative momentum states
    ###       have hole placed at site = 2*system.size (last site of the second chain)

    ### set sublattice rotation masks (we rotate sublattice without the hole in representative states)
    mask = getMask(system)

    ### number of spins pointing up
    nSpinsUp::Int64 = system.size + div(system.magnetization - 1, 2)
    ### note: in the code `spin up === 1`, `spin down === 0`

    ### calculate magnetic subspace size
    subspaceSize::Int64 = binomial(2*system.size - 1, nSpinsUp)
    ...
    ### to introduce momentum states we ned to mix subspaces
    ### with the same abs(#spins_up - #spins_down)
    nSpinsUp = 2*system.size - 1 - nSpinsUp
...
function getMask(system::System)::Int64
    return sum(1 << k for k in 1:2:(system.size - 1)) + sum(1 << k for k in system.size:2:(2*system.size - 1))
...
function viewState(state::Int64, system::System)
    return transpose(reshape(digits(state, base = 2, pad = 2*system.size), system.size, 2))
end
...
"""
    bitmov2(s::Int, d::Int, f::Bool = false; hb::Int = 1 << (d - 1), hv::Int = (1 << d) - 1) -> Int64

Cyclic bit shift for calculationg bit translations with periodic boundary conditions in the subchains of the binary representation of an integer.

# Arguments
*   `d::Int` - size of the cycle (total number of bits in the cycle - subset of the whole bit representation)
*   ...for details of the other aguments see 'bitmov'.
"""
@inline function bitmov2(s::Int, d::Int, f::Bool = false; hb::Int = 1 << (d - 1), hv::Int = (1 << d) - 1)::Int64
    result = 0
    for i in 0:1 # iterate over chains
        take = (2^d)^(i+1) - (2^d)^i
        val = (s & take) >> (d * i)
...
function hamiltonian(state::Int64, basis::Basis, system::System)::LinearCombination
    ...
    result = LinearCombination(fill(state, 3*system.size), zeros(Complex{Float64}, 3*system.size))
    ...
    if haskey(basis, state)
        ...
        ### workout magnons in both chains first
        ### loop over bonds along chain 1
        for i in 1:system.size
            ...
            if (iBit == jBit) && (system.chain2type == 1)
            ...
            magnonMagnon = -1.0 * system.coupling * iBit * jBit
            result.coefficient[1] += magnons + magnonMagnon * system.magnonmagnon2
        end

        ### loop over bonds without the hole along chain 2
        for i in (system.size + 1):(2*system.size - 2)
            j = i + 1
            ...
            ## work out off-diagonal coeffcients
            iBit, jBit = div(state & iValue, iValue), div(state & jValue, jValue)
            if (iBit == jBit) && (system.chain1type == 1)
                ### if two neighbouring spins are different then flip those spins
                ### here we have magnon language so if 2 sites are both empty (or
                ### occupied) by magnons then create (or annihilate) magnons
                newState = xor(state, iValue + jValue)
            ...
            result.coefficient[1] += magnons + magnonMagnon * system.magnonmagnon1
        end

        ### workout sites/bonds around the hole along chain 2
        i = 2*system.size # hole position
        for j in [system.size + 1, 2*system.size - 1]
            ### get bit value at i and j bit positions
            iValue, jValue = (1 << (i - 1)), (1 << (j - 1))
            iBit, jBit = div(state & iValue, iValue), div(state & jValue, jValue)
            ...
            result.coefficient[1] += magnons + hole + holeMagnon * system.holemagnon1
            ...
            result.state[2*system.size - 1 + j] = newState
            result.coefficient[2*system.size - 1 + j] = coefficient * system.hopping
        end

        ### loop over interchain bonds without a hole
        for i in 1:(system.size - 1)
            j = i + system.size
            ...
            ## work out off-diagonal coeffcients
            iBit, jBit = div(state & iValue, iValue), div(state & jValue, jValue)
            if (iBit == jBit) && (system.interchaintype == 1)
                ### if two neighbouring spins are different then flip those spins
                ### here we have magnon language so if 2 sites are both empty (or
                ### occupied) by magnons then create (or annihilate) magnons
                newState = xor(state, iValue + jValue)
                ...
                ### calculate matrix coefficient
                coefficient = 0.5 * system.interchain # there is no exponantial since distance to representative is 0
                if system.periodic == 1
                    coefficient *= 2
                end

                ### put new state in linear combination
                ### and set its corresponding coefficient
                result.state[i + 1 + 2*system.size] = newState
                result.coefficient[i + 1 + 2*system.size] = coefficient
            end

            #### diagonal coefficient
            ## comment: after rotation bits represent magnons (0 -> no magnon, 1 -> magnon present)
            iBit, jBit = div(state & iValue, iValue), div(state & jValue, jValue)
            magnons = 0.5 * system.interchain * (iBit + jBit)
            magnonMagnon = -1.0 * system.interchain * iBit * jBit # note: by definition there is no magnon at the position of the hole
            if system.periodic == 1
                magnons *= 2
                magnonMagnon *= 2
            end
            result.coefficient[1] += magnons + magnonMagnon * system.magnonmagnoninter
        end

        ### workout interchain bonds with a hole
        i = 2*system.size ## hole position
        j = system.size
        ### get bit value at i and j bit positions
        iValue, jValue = (1 << (i - 1)), (1 << (j - 1))
        ...
        #### diagonal coefficient
        magnons = 0.5 * system.interchain * jBit
        hole = 0.5 * system.interchain
        holeMagnon = -0.5 * system.interchain * jBit
        if system.periodic == 1
            magnons *= 2
            hole *= 2
            holeMagnon *= 2
        end
        result.coefficient[1] += magnons + hole + holeMagnon * system.holemagnoninter
...
function makeModel(basis::Basis, system::System)::Model
    ...
    linearCombinationLength = 3*system.size
