...
function getPeriodicity(state::Int64, system::System)::Int64
    ...
    periodicity::Int64 = 1
    while state != (stateTranslation = bitmov2(stateTranslation, l, false, hb = highestBit, hv = highestValue))
        periodicity += 1
    end
...
function getRepresentative(state::Int64, system::System)::Int64
    ...
    newState = state
    while state != (newState = bitmov2(newState, l, false, hb = highestBit, hv = highestValue))
        ...
        for i in 1:(system.size^2)
            ...
            if iBit == jBit
                ...
                if hasMomentum
                    ...
                    ### create a new entry in linear combination
                    ### and set its corresponding coeffcient
                    result.state[i + 1 + system.size^2] = repState
                    result.coefficient[i + 1 + system.size^2] = coefficient * system.intrachain * system.isotropy
                end
            end
            ...
            ## comment: after rotation bits represent magnons (0 -> no magnon, 1 -> magnon present)
            iBit, jBit = div(state & iValue, iValue), div(state & jValue, jValue)
            result.coefficient[1] -= (0.25 - 0.5 * (iBit + jBit) +  iBit * jBit) * system.intrachain
        end
    end
